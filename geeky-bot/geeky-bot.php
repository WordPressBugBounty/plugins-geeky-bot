        ...
        if( $options['action'] == 'update' && $options['type'] == 'plugin' && isset( $options['plugins'] ) ) {
            ...
            foreach( $options['plugins'] as $plugin ) {
                if( $plugin == $our_plugin ) {
                    ...
                    include_once GEEKYBOT_PLUGIN_PATH . 'includes/updates/updates.php';
                    GEEKYBOTupdates::GEEKYBOT_checkUpdates('128');
...
add_action('admin_footer', function() {
    // Ensure the function exists before calling it (safety check)
    ...
    // 1. Define exactly which post types should get the AI Drawer
    $allowed_post_types = [
        'post', 
        'page', 
    ...
    // 2. Check if we are on an editing screen AND the post type is allowed
    if ($screen->base === 'post' && in_array($screen->post_type, $allowed_post_types)) {
        include plugin_dir_path(__FILE__) . 'modules/zywrap/editor_zywrap_drawer.php';
    }
});   
